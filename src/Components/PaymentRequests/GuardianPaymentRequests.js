...
//       .then(async (res) => {
//         console.log(res);
//         console.log(res.data.allPaymentRequests.PaymentRequests[0].itemDetail);
//         console.log(
//           res.data.allPaymentRequests.PaymentRequests[0]
//             .fkPaymentRequestStoreIdrel
//         );
//         console.log(
//           res.data.allPaymentRequests.PaymentRequests[0]
//             .fkPaymentRequestStoreIdrel.Businesses[0].storeName
//         );

//         // let storeName = await res.data.allPaymentRequests.PaymentRequests[0]
//         //   .fkPaymentRequestStoreIdrel.Businesses[0].storeName;
//         let madeBy_name = await res.data.allPaymentRequests.PaymentRequests[0]
//           .fkpaymentrequestcreatebymaprel.Userdata[0].firstname;
//         //console.log("helllo");
//         let array = [],
//           obj1;

//         console.log(res.data.allPaymentRequests.totalCount);
//         console.log(res.data.allPaymentRequests.PaymentRequests[0])
//         for (let i = 0; i < res.data.allPaymentRequests.totalCount - 1; i++) {
//           // let storeName = await res.data.allPaymentRequests.PaymentRequests[i]
//           //   .fkPaymentRequestStoreIdrel.Businesses[0].storeName;
//           let madeBy_name = await res.data.allPaymentRequests.PaymentRequests[i]
//             .fkpaymentrequestcreatebymaprel.Userdata[0].firstname + " " + res.data.allPaymentRequests.PaymentRequests[i]
//             .fkpaymentrequestcreatebymaprel.Userdata[0].lastname;
//             // let madeFor_name = await res.data.allPaymentRequests.PaymentRequests[i]
//           let amount = await res.data.allPaymentRequests.PaymentRequests[i].requestedAmount + "$";
//           let transactionDate = await res.data.allPaymentRequests.PaymentRequests[i].createdAt;
//             //console.log(amount);
//              //console.log(madeFor_name);
//              console.log(transactionDate);
//           obj1 = {
//             //storeName: storeName,
//             madebyName: madeBy_name,
//             //madeFor_name: madeFor_name
//             amount: amount,
...
//             if (!storeName.includes(_storeName)) {
//               storeName.push(_storeName);
//             }
//             return (
//               (item.cardNumber =
...
//                 : "",
//             listData: res?.data?.allPaymentRequests?.PaymentRequests || [],
//             listData_copy: res?.data?.allPaymentRequests?.PaymentRequests || [],
//             storeName: storeName,
//           });
//         }
//       })
...
class GuardianPaymentRequest extends Component {
    ...
    fetchMethod(
      ...
      .then((res) => {
        ...
        if (res && res.error && res.error.statusCode === 401) {
          ...
          res.data.allPaymentRequests.PaymentRequests.map((item) => {
            item["madeby"] = (
              item?.fkpaymentrequestcreatebymaprel?.Userdata[0]?.firstname +
              " " +
              item?.fkpaymentrequestcreatebymaprel?.Userdata[0]?.lastname
            ).trim();
            //item["txnId"] = item.txnId || '-';
            item["madefor"] = (
              item?.fkPaymentRequestParticipantIdrel?.Userdata[0]?.firstname +
              " " +
              item?.fkPaymentRequestParticipantIdrel?.Userdata[0]?.lastname
            ).trim();
            item["amountAdded"] = item.requestedAmount
            ...
            item["createdAt"] =
              ...
              item["createdAt"] = item.createdAt
                ? this.formatDate(item.createdAt).split(" ")[0]
                : "-";
            item["itemDescription"] = item.itemDetail
              ? "$" + item.itemDetail
              : "-";
  ...
  //   // if (type == "all") {
  //   //   let allData = res.data.allPaymentRequests.PaymentRequests.map(
  //   //     function (obj) {
  //   //       return {
  //   //         ["Payment Made For"]: obj.madefor,
  //   //         ["Payment Made By"]: obj.madeby,
  ...
  handleDateFilterautocomplete = (date) => {
    console.log(date.target.value);
    let d = date.target.value.split("-").reverse().join("/");
    // if (event.target.value) {
    //   this.setState({
    //     listData: this.state.listData_copy.filter((user) =>
    //       user.createdAt.includes(event.target.value)
    //     ),
    //   });
    // } else {
    this.setState({
      selectedDate: date,
      listData: this.state.listData_copy.filter((record) =>
  ...
  handleDateFilterautocomplete = (event) => {
    if (event.target.value) {
      this.setState({
        listData: this.state.listData_copy.filter((user) =>
          user.createdAt.includes(event.target.value)
  ...
  handleamountFilterautocomplete = (event) => {
    if (event.target.value) {
      this.setState({
        listData: this.state.listData_copy.filter((user) =>
          user.amountAdded.includes(event.target.value)
        ),
      });
    } else {
      this.setState({
  ...
  handlemadeByFilterautocomplete = (event) => {
    if (event.target.value) {
      this.setState({
        listData: this.state.listData_copy.filter((newuser) =>
          newuser.madeby.includes(event.target.value)
        ),
      });
    } else {
      this.setState({
  ...
  handlemadeForFilterautocomplete = (event) => {
    if (event.target.value) {
      this.setState({
        listData: this.state.listData_copy.filter((newuser) =>
          newuser.madefor.includes(event.target.value)
        ),
      });
    } else {
      this.setState({
  ...
  handleStoreFilterautocomplete = (event) => {
    if (event.target.value) {
      this.setState({
        listData: this.state.listData_copy.filter((user) =>
          user.storeName.includes(event.target.value)
        ),
      });
    } else {
      this.setState({
  ...
  resetFilters = () => {
    ...
    delete this.state.filter.txnType;
    document
      .querySelectorAll(".textFilters .MuiInputBase-input.MuiInput-input")
      .forEach((tag) => {
        tag.value = null;
      });
    this.setState(
  ...
  handleStoreNameAutocomplete = (e, v) => {
    if (v) {
      this.setState({
        listData: this.state.listData_copy.filter(
          (record) => record.storeName === v
        ),
      });
    }
  };

  // handledateAutocomplete = (e, v) => {
  //   if (v) {
  //     this.setState({
  //       listData: this.state.listData_copy.filter(
  //         (record) => record.createdAt === v
  //       ),
  //     });
  //   }
  // };

  handledateAutocomplete = (event) => {
    if (event.target.value) {
      this.setState({
        listData: this.state.listData_copy.filter((user) =>
          user.createdAt.includes(event.target.value)
  ...
  //   if(event.target.value) {
  //     this.setState({
  //       listData: this.state.listData_copy.filter(user => user.createdAt.includes(event.target.value))
  //     });
  //   } else {
  //     this.setState({
  //       listData: this.state.listData_copy
        ...
        <div className="paymentFex">
          ...
          {/* <FormControl className="searchTextField">
            ...
            <Autocomplete
              ...
              // style={{ width: 300 }}
              renderInput={(params) => (
                <TextField {...params} label="Store Name" variant="outlined" />
              )}
            />
          </FormControl> */}
          ...
          {/* <FormControl className="searchTextField">
             <Autocomplete
              ...
              // style={{ width: 300 }}
              renderInput={(params) => (
                <TextField {...params} label="Added On" variant="outlined" />
              )}
            />
          </FormControl> */}
          ...
          {/* <FormControl>
            <Autocomplete
              id="combo-box-demo"
              size="small"
              value={this.state.storexyz}
              options={this.state.storeName}
              onChange={(e, v) => this.handleStoreNameAutocomplete(e, v)}
              getOptionLabel={(option) => option || ""}
              // style={{ width: 300 }}
              renderInput={(params) => (
                <TextField {...params} label="Store Name" />
              )}
            />
          </FormControl> */}
          <FormControl>
            ...
            className="textFilters"
              onKeyUp={(event) => this.handlemadeByFilterautocomplete(event)}
              label="Payment Made By"
            />
          </FormControl>
          ...
          <FormControl>
            <TextField
            className="textFilters"
              onKeyUp={(event) => this.handlemadeForFilterautocomplete(event)}
              label="Payment Made For"
            />
          </FormControl>
          ...
          <FormControl>
            <TextField
            className="textFilters"
              onKeyUp={(event) => this.handleamountFilterautocomplete(event)}
              label="Requested Amount"
            />
          </FormControl>
          ...
          <FormControl>
            ...
            <TextField
            className="textFilters"
              onKeyUp={(event) => this.handleStoreFilterautocomplete(event)}
              label="Store Name"
            />
          </FormControl>
          ...
          <FormControl>
            <TextField
              className="textFilters"
              onKeyUp={(event) => this.handleDateFilterautocomplete(event)}
            ...
            {/* <TextField
              ...
              type="date"
              format="MM/dd/yyyy"
              onChange={(evt) => this.handleDateFilterautocomplete(evt)}
              InputLabelProps={{
                shrink: true,
              }}
            /> */}
          ...
          {/* <FormControl>
            <Autocomplete
              ...
              value={this.state.storexyz}
              options={this.state.storeName}
              onChange={(e, v) => this.handledateAutocomplete(e, v)}
              getOptionLabel={(option) => option || ""}
              // style={{ width: 300 }}
              renderInput={(params) => (
                <TextField {...params} label="Transaction Date" />
              )}
            />
          </FormControl> */}
