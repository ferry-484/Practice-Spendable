...
class FormComponent extends Component {
    ...
    if (textTemp[0] == "1" || textTemp[0] == "0") {
      if (textTemp.length === 2) {
        if (
          ...
          if (
            textTemp.includes("/") ||
            textTemp.length < formData[name].length
          ) {
          } else {
            textTemp += "/";
          }
      ...
      if (this.props.formConfig.dependentValidations[name] != undefined) {
        ...
        if (errors[name]) {
          errorMessages[name] = this.props.formConfig.dependentValidations[
            name
          ](textTemp, this.state.formData).errMsg;
        } else {
          delete errorMessages[name];
        }
      }
  ...
  renderFields = formConfig => {
    return formConfig.fields.map((item, index) => {
      switch (item.type) {
          ...
          return (
            <div key={index}>
              <TextField
                ...
                // onChange={this.handleDateTimeChange.bind(this, item.name)}
        ...
        case "addStoreTypeButton":
          return(
            <Button key={index} onClick={() => this.addStoreTypeField(index)}>
                Add More
            </Button>
          );
        case "deleteStoreTypeButton":
          return(
            <IconButton key={index} className="deleteStoreIcon" 
            onClick={() => this.deleteStoreTypeField(index)}>
  ...
  submitHandler = () => {
    ...
    if (noErrors) {
      formData['storeType'] = [...new Set(this.prepareStoreTypeData(formData))];
  ...
  prepareStoreTypeData = (formData) => {
    let storeType = [];
    Object.keys(formData).map(key => {
      if(key.includes('storeType') && formData[key].id) {
        storeType.push(formData[key].id);
      }
    });
    return storeType;
  }

  addStoreTypeField = (index) => {
    console.log("state", this.state, this.props);
    this.props.formConfig.fields.splice(index, 0, {
      label: "Store Type",
      type: "searchSelect",
    ...
    this.props.formConfig.validations["storeType" + index] = e => {
      if (e == "") {
        return {
          valid: false,
          errMsg: "Please choose store type"
        };
      }
      return {
        valid: true
  ...
  deleteStoreTypeField = (index) => {
    delete this.state.formData[this.props.formConfig.fields[index-1].name];
    this.setState({
      formData: this.state.formData
    })
    this.props.formConfig.fields.splice(index-1, 2)
    this.initializeForm();
  }
